...
apt-get install -y tzdata locales snapd \
  ttf-wqy-microhei fonts-arphic-uming fonts-arphic-ukai \
  sudo software-properties-common psmisc net-tools iputils-ping lsof dnsutils \
  inotify-tools gnupg2 less vim-nox zip unzip unrar p7zip-full screen \
  openssh-server man-db manpages-dev source-highlight git subversion \
...
BUILDER_ID=54321
USER_HOME="/mnt/$BUILDER"
useradd -d $USER_HOME -s /bin/bash -g staff -G sudo -N -m -u $BUILDER_ID \
